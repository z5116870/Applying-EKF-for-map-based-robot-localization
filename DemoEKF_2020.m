% Author: Roark Menezes
% Program: DemoEKF.m - version 2020.T1, Solution for Project2.Part1
% MRTN4010 - S1.2020
% Example using Extended Kalman Filter (EKF) for the localization problem, using LIDAR range observations.
% This will be incorporated in project 1's LaserProcess.m file to produce
% the estimated path using EKF instead of Dead Reckoning.
function main()
...
for i=1:Li,     % loop
    ...
    % Pu is the input noise, calculated using sdevs of speed and gyro
    % (inputs to process model)
    Pu = [stdDevSpeed^2, 0; 0, stdDevGyro^2];
    % Fu
   ...
   % The predition step, for this iteration, is done.
    ...
    if nDetectedLandmarks>0,     % any laser data and detected landmarks?
        ...
        for u=1:nDetectedLandmarks,
            ...
            z(1, :)  = MasuredRanges(u) - ExpectedRange ;   
            z(2, :) = MeasuredBearings(u) - ExpectedBearing;
            z(2, :) = wrapToPi(z(2, :));
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %add measuredheading - expected heading%%%   
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % ------ covariance of the noise/uncetainty in the measurements
             R = [sdev_rangeMeasurement^2, 0; 0, (sdev_bearingMeasurement*pi/180)^2];
...
function InitSimulation(stdDevSpeed,stdDevGyro,sdev_rangeMeasurement,DtObservations,sdev_bearingMeasurement)
    ...
    ContextSimulation.sdev_bearingMeasurement = sdev_bearingMeasurement*pi/180;
...
function [nDetectedLandmarks,MasuredRanges,IDs,MeasuredBearings]=GetObservationMeasurements(map)
    ...
    % simulated platform.
        ...
        if (nDetectedLandmarks<1)       % no detected landmarks...
            MasuredRanges=[];   IDs=[];  RealBearings=[];  return ; 
        end;
        ...
        noiseInBearings = ContextSimulation.sdev_bearingMeasurement*(pi/180)*randn(size(RealBearings));
